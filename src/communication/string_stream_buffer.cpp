...
public:
	...
	std::string_view read_next_chunk(size_t chunk_size)
	{
...
#else
		if (!mapped_data_ || map_size != last_chunk_size_)
		{
			if (mapped_data_)
			{
				munmap(mapped_data_, last_chunk_size_);
			}
			mapped_data_ = static_cast<char*>(mmap(nullptr, map_size, PROT_READ, MAP_SHARED, fd_, read_offset_));
		...
		std::string_view chunk(mapped_data_, map_size);
		read_offset_ += map_size;
		last_chunk_size_ = map_size;
...
private:
	...
	size_t allocation_granularity_;  // Windows-specific
